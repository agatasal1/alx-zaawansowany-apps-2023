...
var _tasks = require("./shared/tasks");
...
const taskId = (0, _helpersDefault.default)(window.location.search);
const renderTask = (task)=>{
    detailTodoForm.elements.task.value = task.task;
    detailTodoForm.elements.date.value = task.date;
};
(0, _tasks.getTask)(taskId).then((task)=>{
    renderTask(task);
});
const saveTask = (task)=>{
...
const handleTaskChange = (event)=>{
    ...
    return saveTask(editedTask).then(()=>{
        window.location.href = "/index.html";
    });
};
changeTaskButton.addEventListener("click", handleTaskChange);

},{"./shared/tasks":"10Sg6","./shared/helpers":"9G7TO","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"10Sg6":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "getTasks", ()=>getTasks);
parcelHelpers.export(exports, "getTask", ()=>getTask);
const getTasks = ()=>{
    return fetch("http://localhost:8000/tasks").then((response)=>{
        return response.json();
    });
};
const getTask = (id)=>{
    return fetch(`http://localhost:8000/tasks/${id}`).then((res)=>res.json());
};
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["a3yxP","hPCZF"], "hPCZF", "parcelRequire72f7")
